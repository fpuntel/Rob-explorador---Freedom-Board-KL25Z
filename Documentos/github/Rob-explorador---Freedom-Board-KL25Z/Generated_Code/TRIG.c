...
**     Date/Time   : 2013-09-09, 16:45, # CodeGen: 50
**     Abstract    :
...
void TRIG_SetDir(LDD_TDeviceData *DeviceDataPtr, bool Dir)
{
  ...
  if (Dir) {
    /* Output */
    GPIO_PDD_SetPortOutputDirectionMask(TRIG_MODULE_BASE_ADDRESS, TRIG_PORT_MASK);
  } else {
    /* Input */
    GPIO_PDD_SetPortInputDirectionMask(TRIG_MODULE_BASE_ADDRESS, TRIG_PORT_MASK);
  }
}
...
bool TRIG_GetVal(LDD_TDeviceData *DeviceDataPtr)
{
  ...
  if ((GPIO_PDD_GetPortDirection(TRIG_MODULE_BASE_ADDRESS) & TRIG_PORT_MASK) == 0U) {
    /* port is configured as input */
    portData = GPIO_PDD_GetPortDataInput(TRIG_MODULE_BASE_ADDRESS) & TRIG_PORT_MASK;
  } else {
